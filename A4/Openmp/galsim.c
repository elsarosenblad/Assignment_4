...
// Function prototypes
void get_force_on_body(const int nstars, const double G, const double e0, Vector2D* position, double* mass, double* ACCx, double* ACCy, int NUM_THREADS);
void update_velocity_and_position(const double stepsize, const int nstars, Vector2D* velocity, Vector2D* position, double* mass, double* ACCx, double* ACCy, int NUM_THREADS);
...
int main(int argc, char* argv[]) {
    ...
    for (int time = 0; time < nsteps; time++) {
        get_force_on_body(nstars, G, e0, position, mass, ACCx, ACCy, NUM_THREADS);
        update_velocity_and_position(stepsize, nstars, velocity, position, mass, ACCx, ACCy, NUM_THREADS);
    }
